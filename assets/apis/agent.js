$(document).ready(function() {
    ...
    $("#customSwitch-12").on('change', function(){
        var result = '';
        var switchStatus = '';
        if ($(this).is(':checked')) {
            switchStatus = $(this).is(':checked');
            switchStatus = 'APPROAVED';
            $("#customSwitch-12" ).prop( "checked", true );
        }
        else {
           switchStatus = $(this).is(':checked');
           switchStatus = 'REJECTED';
           $("#customSwitch-12" ).prop( "checked", false);
        }
        ...
        if(switchStatus != ''){
            $.ajax({
                ...
                url: ip_address+'/api/advert/create',
                contentType: 'application/json;charset=UTF-8',
                data: JSON.stringify({'campaign_id':window.localStorage.getItem('aid'),'status':switchStatus}),
                dataType: "json",
                cache: false,
                processData: false,
                success: function(data) {
                    ...
                    if (data.status!=200){
                        ...
                        +'<p style="font-size:12px;color:red;font-weight:bold;width:200px;margin-left:-75px;">Rejected</p>'
                        +'</div>'
                        $("#status_success").attr("data-message", n);
                        $('#status_success').click();
                        document.getElementById('b-loader').style.display = 'none';
